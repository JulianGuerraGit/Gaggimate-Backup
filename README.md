...
History export downloads or reuses `index.bin` and all `.slog` files first. After the shot files are complete, it runs a separate note pass through the WebSocket notes API and writes padded note files like `000001.json`.
